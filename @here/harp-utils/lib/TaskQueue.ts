...
export interface Task {
    /**
     * The Function that will executed when the [[Task]] is processed
     */
    execute: () => void;
...
/**
 * This is a helper interface to be able to index the TaskArrays via a string
 */
interface TaskArrays {
    [key: string]: Task[];
}
...
export class TaskQueue {
    private m_taskArrays: TaskArrays = {};

    constructor(private m_options: TaskQueueOptions) {
        this.m_options.groups?.forEach(group => {
            this.m_taskArrays[group] = [];
        });
        this.sort = this.m_options.prioSortFn ?? this.sort;
    }

    /**
     * Updates the lists in the queue depending on their priority functions
     * Needs to be called by using instance, as it would be to costly on every change
     * @param group The Group to update, if not set all groups will be updated.
     */
    update(group?: string) {
        if (group === undefined) {
            this.forAllGroups(taskList => {
    ...
    add(task: Task): boolean {
        if (this.m_taskArrays[task.group]) {
            this.m_taskArrays[task.group].push(task);
            return true;
        }
        return false;
    }
    ...
    remove(task: Task): boolean {
        if (this.m_taskArrays[task.group]) {
            const index = this.m_taskArrays[task.group].indexOf(task);
            if (index !== -1) {
                this.m_taskArrays[task.group].splice(index, 1);
                return true;
            }
        }
        return false;
    ...
    private sort(a: Task, b: Task): number {
        ...
        return b.getPrio() - a.getPrio();
    }

    private getTaskList(group: string): Task[] | undefined {
        return this.m_taskArrays[group];
    }
    ...
    private forAllGroups(fn: (tasklist: Task[]) => void) {
        for (const key in this.m_taskArrays) {
            if (this.m_taskArrays[key]) {
                fn(this.m_taskArrays[key]);
            }
        }
    }
}
